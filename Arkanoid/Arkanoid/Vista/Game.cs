        ...
        private void LoadTiles()
        {
            ...
            for (int i = 0; i < y; i++)
            {
                for (int j = 0; j < x; j++)
                {
                    ...
                    int[] numRandom;
                    numRandom = new int[14];
                    
                    for (int k = 0; k < 5; k++)
                    {
                        for (int l = 0; l < 3; l++)
                        {
                            cpb[i, j].BackgroundImage = Image.FromFile("../../tiles/" + GenerateBlnded() + ".png");
                            cpb[i, j].Tag = "blinded";
                        }
                    } 
                ...
                for (int j = 0; j < 10; j++)
                {
                    if (cpb[i, j] != null && ball.Bounds.IntersectsWith(cpb[i, j].Bounds))
                    {
                            ...
                            if (cpb[i, j].Hits == 0)
                        {
                            ...
                            {
                                if (cpb[i, j].BackgroundImage.Equals("12.png")) 
                                {
                                    cpb[i, j].BackgroundImage = Image.FromFile("../../tiles/brk.png");
                                }
                               else if ( cpb[i, j].BackgroundImage.Equals("13.png"))
                                {
